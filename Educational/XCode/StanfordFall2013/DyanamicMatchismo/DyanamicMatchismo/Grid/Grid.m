...
{
    NSString *description = [NSString stringWithFormat:@"[%@] fitting %lu cells with aspect ratio %g into %@ -> ",
                                    NSStringFromClass([self class]),
                                        (unsigned long)self.minimumNumberOfCells,
    ...
    if (!self.rowCount) {
        ...
        if (!self.minimumNumberOfCells || !self.cellAspectRatio || !self.size.width || !self.size.height) {
            ...
            if (self.minCellWidth || self.minCellHeight) {
                ...
                if (self.minCellWidth && self.minCellHeight) {
                    description = [description stringByAppendingFormat:@" (minCellWidth = %g, minCellHeight = %g)",
                                                                        self.minCellWidth, self.minCellHeight];
        ...
        description = [description stringByAppendingFormat:@"%luc x %lur at %@ each",
                                        (unsigned long)self.columnCount, (unsigned long)self.rowCount,

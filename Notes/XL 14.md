...
### Backend
1. Fix the `AJAX` response from the server to be (potentially) `JSON` suitable for use updating the `DOM` using the `jQuery` code below.
2. Create a `tasks#all` route, controller action, and view
3. Move the `Create` buttons to `AJAX`

### Frontend
1. A potential fix for the `AJAX` on the view-side is code similar to the following (from *Yavascript Whackamole*)
...
1. Have a `Tasks` homepage with navbar link, like for `Categories`
2. Check out [DataTables](https://datatables.net), a `jQuery` framework
	3. not sure how to include it, but it probably just involves adding it to `application.js` or something
	3. Use it to display all tasks due within the next 2 weeks on the user's homepage
...
1. Use the Rails books to figure out what sorts of `validations` I ought to be doing
6. Create **partials** for displaying the models, as seen in *Beginning Rails 4, 3rd Ed.*
...
2. To display the `tasks` belonging to the `@category` we are `show`ing, you can do

        <%= render @tasks %>
    
    1. First you have to define `@tasks = @category.tasks` in the controller.
    2. Now for each `task` it will look for a partial called `app/views/tasks/_task.html.erb`
    3. Now in that `erb`, put
    
            <%= div_for task do %>
                <%= task.name %>
            <% end %>

        * Here, `div_for` is going to create
                
                <div id="task_4" class="task">Name</div>

    4. Now in `app/assets/stylesheets/application.scss`, put
...
3. In `task.rb`, adding the line

        default_scope { order("duedate ASC") }

    ought to make it so that getting lists of tasks are by default sorted by nearest `duedate` first
    
4. Add validations such as in `task.rb` (after finding out what exactly they do and don't do)

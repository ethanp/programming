...
1. Fix the **timezone crap** in `duedates` and `intervals`

### Frontend

1. Figure out some good way to indicate (through coloring?) how urgent a `Task` is
...
4. List the `@task.subtasks`
5. List the `@task.intervals`
6. Figure out how to add the **Start/End Timer** button.
    * Using javascript?
    * I have no idea.

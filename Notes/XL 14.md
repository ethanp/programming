...
3. Re-listen to the instructions in the *How to Show_Hide Links When a User Is Signed Into a Rails App* youtube video
...
1. Better `f.datetime_picker` styling in the (2) `task` forms!
2. I've got a new video saved about how to include Bootstrap 3 by copy-pasting files.
    3. It seems like the consensus of the 3 people I randomly saw on the internet was that
       this is the best way to do it. So fine; I'll do it that way.
...
1. *has many* **time intervals**: cascade delete
...
## Next time with Rails

1. Follow the video advice I saved (red) for copy-pasting the bootstrap 3 files into the lib
    1. Certainly, I could also pull this off in the current one
    2. Just start a blank project and seeing what I need to bring back to make the current one complete.
2. Use nested resources, then you can use `shallow: true` to get better URLs, see [StOve nesting][]
    1. That will completely get rid of my issue of setting which one I'm doing it for, which is really the only hassle I'm having right now and it's a real pain in the butt.
    2. Actually, I might be able to pull this off in the current one, it looks like the *only* thing that changes is the `routes.rb`?


[StOve nesting]: http://stackoverflow.com/questions/10661315/rails-3-level-deep-nested-resources
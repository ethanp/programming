...
## NOTE!
### I should be using the GitHub Issue Trracker for this

## High
### Backend

1. Support/allow for **Task name lengths** up to 60 chars in length
    * 30 is just too darn short
2. Edit Task -> Back button -> "Page does not exist"
3. There should be a page for *complete tasks* too

### Frontend

1. Change each category panel from col-md-4 to col-md-3 and put them in rows grouped in 4's like
...
2. `Show/Hide Intervals` should *Show/Hide* the `Intervals`, *not* the Form`
3. `+` & `-` buttons on the live Interval recorder
4. A way to have a link to the assignment/reading in question
5. In `/tasks`, sorting by `Duedate`, actually sorts alphabetically
   by day of the week
    * This might help: *http://www.datatables.net/forums/discussion/2467/need-help-for-sorting-date-with-dd-mm-yyyy-format*
6. When you `edit()` a `Task`
    1. It leaves the `Duedate` field blank
        * It *should* be filled with the *current* `Duedate` value
    2. It defaults the `Category` to the *first* option, rather than the *existing* option
    3. Same for `Priority`
7. Do Javascript Validation of forms or at least put the field values
   back when it gets rejected by the validations
8. **Time display issue** -- there is something wrong with the way
...
9. Add **HTML page titles** so that the *back* button reveals where
   it'll take you


### Charts

1. A normal line chart [like Bostock's](http://bl.ocks.org/mbostock/3884955)
    1. Just put it underneath the existing chart --- the transitions are not that important right now
2. A line-graph for some kind of *"Score"* that combines the amount of time spent doing different things of different priorities

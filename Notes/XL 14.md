...
1. *Bostock's* example uses a *TSV* of `Day:Int | Hour:Int | Value:Int`
2. For me, the most convenient would be the same but `Value:Float`. It shouldn't make a difference.
3. I suppose one relatively classy way to send the data is to make the `task_2_heatmap.tsv` a `Route`
    1. For simplifying the integration with `Rails`, maybe I ought to just use `task_2_heatmap.json`

#### Frontent: Render the data
1. Well...it ought to basically *just work*
...
3. Move the `Create` buttons on the `tasks/show.html` to `AJAX` and hook it into validations (after finding out what exactly they do and don't do from the book)

...
4. Support/allow for **Task name lengths** up to 60 chars in length
    * 30 is just too darn short 

### Frontend

1. A way to have a link to the assignment/reading in question
1. In `/tasks`, sorting by `Duedate`, actually sorts alphabetically
   by day of the week
    * This might help: *http://www.datatables.net/forums/discussion/2467/need-help-for-sorting-date-with-dd-mm-yyyy-format*
1. When you `edit()` a `Task`, it leaves the `Duedate` field blank
    * It *should* be filled with the *current* `Duedate` value
1. Do Javascript Validation of forms or at least put the field values
   back when it gets rejected by the validations
2. **Time display issue** -- there is something wrong with the way
...
3. Add **HTML page titles** so that the *back* button reveals where
   it'll take you
...
1. A line-graph for some kind of *"Score"* that combines the amount of time spent doing different things of different priorities
...
4. A **useful validation** would be to ensure that for `Interval`, \\(start < end\\)
5. **`estTimeRemaining` attribute for `Task`** --- can be `NULL`?
6. **Search bar in navbar** that searches for matching `Tasks` and `Categories`
7. Add **recurring tasks**
9. Add *"clippy"* or whatever that nice file-attachment module is to

...
#### Backend: Format the data
1. Make it available from the `routes/controller`

#### Frontent: Render the data
1. Perhaps the code for this ought to live in `helpers/application_helper.rb`? Not sure yet. Look in to that. Can do bro.

### Backend

1. Run around like a madman adding validations and scopes

        # e.g. for Subtask
        validates :name, presence: true        
        validates_uniqueness_of :name, scope: [:task_id]
        validates_length_of :name, maximum: 30, too_long: 'That name is too long, (30 characters max)'
        
        # e.g. for Task
        scope :due_within_two_weeks, -> { published.where("articles.published_at > ?", 2.weeks.from_now.to_date)}
        scope :due_before, ->(datetime) { where("duedate < ?", datetime) }
        
   and whatever other useful validations are in the documentation, because that's going to
   make the form return the validation errors, instead of letting the failed Database save
   crash the app.
   1. One useful validation might be ensuring that for `Interval`, `start < end`
   2. "Scoping allows you to specify commonly-used queries which can be referenced as method calls on the association objects or models"
...
3. Add `class Category` `has_many :tasks, order: "duedate"` and `class Task` `has_many :intervals, order: "start"`
### Frontend

1. Add a display of the running `Interval` time in `tasks/show.html#interval-table`
    * Take hints from how they implemented it in that knockoff from Sunnyvale
